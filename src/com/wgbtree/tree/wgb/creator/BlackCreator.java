...
public final class BlackCreator {
    ...
    public static <K extends Comparable<K>, T>
    FwMersenneBlack<K, T> createMersenneFw(K key, Set<T> value, TreeConfig config) {
        var black = new FwMersenneBlack<K, T>(config.getOrder(), config.getPower(), config.getDuplicatesAllowed());
        black.getEntries().add(new SimpleEntry<>(key, value));
        return black;
    }
    ...
    public static <K extends Comparable<K>, T>
    Black<K, T> createRange(K key, Set<T> value, TreeConfig config) {
        var black = new RangeBlack<K, T>(config.getOrder(), config.getDuplicatesAllowed());
        black.getEntries().add(new SimpleEntry<>(key, value));
        return black;
    }
}
