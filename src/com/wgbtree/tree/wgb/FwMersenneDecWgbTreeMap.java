...
import static com.wgbtree.tree.wgb.constants.Constants.DEFAULT_ARE_DUPLICATES_ALLOWED;
import static com.wgbtree.tree.wgb.constants.Constants.DEFAULT_ORDER;
import static com.wgbtree.tree.wgb.model.info.GrowthMode.FW_MERSENNE_ACCELERATING;
import static com.wgbtree.tree.wgb.model.info.GrowthMode.FW_MERSENNE_DECELERATING;
import static com.wgbtree.tree.wgb.prime.Primes.FIRST_MERSENNE_EXP;
import static java.util.Objects.isNull;

public class FwMersenneDecWgbTreeMap<K extends Comparable<K>, T> extends WGBTreeMap<K, T> implements Serializable, AsTree<K, T> {

    private static final boolean DEFAULT_IS_BALANCED = false;

    public FwMersenneDecWgbTreeMap() {
    ...
    public FwMersenneDecWgbTreeMap(int order) {
        super(TreeConfigCreator.createMersenne(
                assertOrder(order),
                FIRST_MERSENNE_EXP,
                DEFAULT_ARE_DUPLICATES_ALLOWED,
    ...
    public FwMersenneDecWgbTreeMap(int order, int power) {
        super(TreeConfigCreator.createMersenne(
                assertOrder(order),
    ...
    public FwMersenneDecWgbTreeMap(int order, int power, boolean duplicatesAllowed) {
        super(TreeConfigCreator.createMersenne(
                assertOrder(order),
                assertMersenneExp(power),
                duplicatesAllowed,
    ...
    public WGBTreeMap<K, T> cloneEmpty() {
        return new FwMersenneDecWgbTreeMap<>(config.getOrder(), config.getPower(), config.getDuplicatesAllowed());
    }
