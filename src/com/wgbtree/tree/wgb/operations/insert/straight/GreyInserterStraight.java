package com.wgbtree.tree.wgb.operations.insert.straight;
...
public final class GreyInserterStraight {
	...
	Grey<K, T> insert(Grey<K, T> grey, K key, Set<T> value, int keyHash, AtomicReference<T> oldValue, TreeConfig config) {
		...
		if (entries.lastEntry().getKey().compareTo(key) < 0) {
			...
			return GreyRotatorStraight.tryRotateLeft(grey, config);
		}

		if (entries.firstEntry().getKey().compareTo(key) > 0) {
			insertLeft(grey, key, value, keyHash, oldValue, config);
			return GreyRotatorStraight.tryRotateRight(grey, config);
		}
	...
	Grey<K, T> insertLeft(Grey<K, T> grey,
						  ...
						  TreeConfig config) {
		var left = insert((Grey<K, T>) grey.getLeft(), key, value, keyHash, oldValue, config);
	...
	Grey<K, T> insertRight(Grey<K, T> grey,
						   ...
						   AtomicReference<T> oldValue,
						   TreeConfig config) {
		var right = insert((Grey<K, T>) grey.getRight(), key, value, keyHash, oldValue, config);
