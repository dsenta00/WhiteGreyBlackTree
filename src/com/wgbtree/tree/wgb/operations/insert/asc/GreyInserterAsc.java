package com.wgbtree.tree.wgb.operations.insert.asc;
...
import com.wgbtree.tree.wgb.operations.rotate.asc.GreyRotatorAsc;
...
public final class GreyInserterAsc {
	...
	Grey<K, T> insert(Grey<K, T> grey, K key, Set<T> value, int keyHash, AtomicReference<T> oldValue, TreeConfig config) {
		...
		if (entries.lastEntry().getKey().compareTo(key) < 0) {
			...
			return GreyRotatorAsc.tryRotateLeft(grey, config);
		}

		if (entries.firstEntry().getKey().compareTo(key) > 0) {
			insertLeft(grey, key, value, keyHash, oldValue, config);
			return GreyRotatorAsc.tryRotateRight(grey, config);
		}
	...
	Grey<K, T> insertLeft(Grey<K, T> grey,
						  ...
						  TreeConfig config) {
		var left = WhiteInserterAsc.insert((White<K, T>) grey.getLeft(), key, value, keyHash, oldValue, config);
	...
	Grey<K, T> insertRight(Grey<K, T> grey,
						   ...
						   AtomicReference<T> oldValue,
						   TreeConfig config) {
		var right = BlackInserterAsc.insert((Black<K, T>) grey.getRight(), key, value, keyHash, oldValue, config);
