...
import com.wgbtree.tree.wgb.operations.rotate.dec.GreyRotatorDec;
...
public final class GreyInserterDec {
	...
	Grey<K, T> insert(Grey<K, T> grey, K key, Set<T> value, int keyHash, AtomicReference<T> oldValue, TreeConfig config) {
		...
		if (entries.lastEntry().getKey().compareTo(key) < 0) {
			...
			return GreyRotatorDec.tryRotateLeft(grey, config);
		}

		if (entries.firstEntry().getKey().compareTo(key) > 0) {
			insertLeft(grey, key, value, keyHash, oldValue, config);
			return GreyRotatorDec.tryRotateRight(grey, config);
		}
	...
	Grey<K, T> insertLeft(Grey<K, T> grey,
		...
		if (config.reachedRankLimit()) {
			...
			left = WhiteInserterDec.insert((White<K, T>) left, key, value, keyHash, oldValue, config);
		}
	...
	Grey<K, T> insertRight(Grey<K, T> grey,
		...
		if (config.reachedRankLimit()) {
			...
			right = BlackInserterDec.insert((Black<K, T>) right, key, value, keyHash, oldValue, config);
		}
