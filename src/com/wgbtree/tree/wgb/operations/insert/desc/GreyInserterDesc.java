package com.wgbtree.tree.wgb.operations.insert.desc;
...
import com.wgbtree.tree.wgb.operations.rotate.desc.GreyRotatorDesc;
...
public final class GreyInserterDesc {
	...
	Grey<K, T> insert(Grey<K, T> grey, K key, Set<T> value, int keyHash, AtomicReference<T> oldValue, TreeConfig config) {
		...
		if (entries.lastEntry().getKey().compareTo(key) < 0) {
			...
			return GreyRotatorDesc.tryRotateLeft(grey, config);
		}

		if (entries.firstEntry().getKey().compareTo(key) > 0) {
			insertLeft(grey, key, value, keyHash, oldValue, config);
			return GreyRotatorDesc.tryRotateRight(grey, config);
		}
	...
	Grey<K, T> insertLeft(Grey<K, T> grey,
		...
		if (isNull(left)) {
			if (config.shouldHaveGreyChildren()) {
				left = GreyCreator.create(key, value, config);
			} else {
				left = WhiteCreator.create(key, value, config);
			}
		} else if (left instanceof Grey<K, T> leftAsGrey) {
			left = insert(leftAsGrey, key, value, keyHash, oldValue, config);
		} else if (left instanceof White<K, T> leftAsWhite) {
			left = WhiteInserterDesc.insert(leftAsWhite, key, value, keyHash, oldValue, config);
		} else {
	...
	Grey<K, T> insertRight(Grey<K, T> grey,
		...
		if (isNull(right)) {
			if (config.shouldHaveGreyChildren()) {
				right = GreyCreator.create(key, value, config);
			} else {
				right = BlackCreator.create(key, value, config);
			}
		} else if (right instanceof Grey<K, T> rightAsGrey) {
			right = insert(rightAsGrey, key, value, keyHash, oldValue, config);
		} else if (right instanceof Black<K, T> rightAsBlack) {
			right = BlackInserterDesc.insert(rightAsBlack, key, value, keyHash, oldValue, config);
		} else {
