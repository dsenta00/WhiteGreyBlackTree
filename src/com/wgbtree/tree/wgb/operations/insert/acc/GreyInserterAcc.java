...
import com.wgbtree.tree.wgb.operations.rotate.acc.GreyRotatorAcc;
...
public final class GreyInserterAcc {
	...
	Grey<K, T> insert(Grey<K, T> grey, K key, Set<T> value, int keyHash, AtomicReference<T> oldValue, TreeConfig config) {
		...
		if (entries.lastEntry().getKey().compareTo(key) < 0) {
			...
			return GreyRotatorAcc.tryRotateLeft(grey, config);
		}

		if (entries.firstEntry().getKey().compareTo(key) > 0) {
			insertLeft(grey, key, value, keyHash, oldValue, config);
			return GreyRotatorAcc.tryRotateRight(grey, config);
		}
	...
	Grey<K, T> insertLeft(Grey<K, T> grey,
						  ...
						  TreeConfig config) {
		var left = WhiteInserterAcc.insert((White<K, T>) grey.getLeft(), key, value, keyHash, oldValue, config);
	...
	Grey<K, T> insertRight(Grey<K, T> grey,
						   ...
						   AtomicReference<T> oldValue,
						   TreeConfig config) {
		var right = BlackInserterAcc.insert((Black<K, T>) grey.getRight(), key, value, keyHash, oldValue, config);
