...
public final class WhiteRemoverMersenneFw {

    public static <K extends Comparable<K>, T>
    RemoveResult<K, T> remove(FwMersenneWhite<K, T> white, K key, int keyHash) {
        if (white == null) {
        ...
        if (removedEntry != null) {
            ...
            if (lastEntry != null && lastEntry.getKey().compareTo(key) < 0) {
                ...
                if (grey instanceof FwGrey<K, T> fwGrey) {
                    var result = GreyRemoverMersenneFw.remove(fwGrey, key, keyHash);
    ...
    void suckMinFromGreyNodes(White<K, T> white) {
        ...
        var minResult = GreyGetterMersenneFw.getMin(greys);
