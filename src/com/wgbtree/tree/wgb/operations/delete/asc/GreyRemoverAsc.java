...
public final class GreyRemoverAsc {
	...
	Entry<K, Set<T>> removeFromRight(@NonNull Grey<K, T> grey, K key, int keyHash) {
		var result = BlackRemoverAsc.remove((Black<K, T>) grey.getRight(), key, keyHash);
	...
	Entry<K, Set<T>> removeFromLeft(@NonNull Grey<K, T> grey, K key, int keyHash) {
		var result = WhiteRemoverAsc.remove((White<K, T>) grey.getLeft(), key, keyHash);
	...
	Entry<K, Set<T>> removeMaxFromLeft(@NonNull Grey<K, T> grey) {
		var maxResult = WhiteRemoverAsc.removeMax((White<K, T>) grey.getLeft());
	...
	Entry<K, Set<T>> removeMinFromRight(@NonNull Grey<K, T> grey) {
		var minResult = BlackRemoverAsc.removeMin((Black<K, T>) grey.getRight());
