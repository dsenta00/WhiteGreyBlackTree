package com.wgbtree.tree.wgb.operations.rotate.power;
...
public final class GreyRotatorPower {
	...
	Grey<K, T> tryRotateLeft(Grey<K, T> grey, TreeConfig config) {
		...
		var entry = GreyRemoverPower.removeMinFromRight(grey);
		var leakedEntry = GreyInserterPower.insertHere(grey, entry);
		...
		return GreyInserterPower.insertLeft(grey, leakedKey, leakedValue, leakedKeyHash, new AtomicReference<>(), config);
	}
	...
	Grey<K, T> tryRotateRight(Grey<K, T> grey, TreeConfig config) {
		...
		var entry = GreyRemoverPower.removeMaxFromLeft(grey);
		var leakedEntry = GreyInserterPower.insertHere(grey, entry);
		...
		return GreyInserterPower.insertRight(grey, leakedKey, leakedValue, leakedKeyHash,new AtomicReference<>(), config);
	}
}
