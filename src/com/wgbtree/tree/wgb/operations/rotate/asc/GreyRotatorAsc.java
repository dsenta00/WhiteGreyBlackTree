package com.wgbtree.tree.wgb.operations.rotate.asc;
...
public final class GreyRotatorAsc {
	...
	Grey<K, T> tryRotateLeft(Grey<K, T> grey, TreeConfig config) {
		...
		var entry = GreyRemoverAsc.removeMinFromRight(grey);
		var leakedEntry = GreyInserterAsc.insertHere(grey, entry);
		...
		return GreyInserterAsc.insertLeft(grey, leakedKey, leakedValue, leakedKeyHash, new AtomicReference<>(), config);
	}
	...
	Grey<K, T> tryRotateRight(Grey<K, T> grey, TreeConfig config) {
		...
		var entry = GreyRemoverAsc.removeMaxFromLeft(grey);
		var leakedEntry = GreyInserterAsc.insertHere(grey, entry);
		...
		return GreyInserterAsc.insertRight(grey, leakedKey, leakedValue, leakedKeyHash,new AtomicReference<>(), config);
	}
}
