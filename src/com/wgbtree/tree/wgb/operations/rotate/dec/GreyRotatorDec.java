package com.wgbtree.tree.wgb.operations.rotate.dec;
...
public final class GreyRotatorDec {
	...
	Grey<K, T> tryRotateLeft(Grey<K, T> grey, TreeConfig config) {
		...
		var entry = GreyRemoverDec.removeMinFromRight(grey);
		var leakedEntry = GreyInserterDec.insertHere(grey, entry);
		...
		return GreyInserterDec.insertLeft(grey, leakedKey, leakedValue, leakedKeyHash, new AtomicReference<>(), config);
	}
	...
	Grey<K, T> tryRotateRight(Grey<K, T> grey, TreeConfig config) {
		...
		var entry = GreyRemoverDec.removeMaxFromLeft(grey);
		var leakedEntry = GreyInserterDec.insertHere(grey, entry);
		...
		return GreyInserterDec.insertRight(grey, leakedKey, leakedValue, leakedKeyHash,new AtomicReference<>(), config);
	}
}
