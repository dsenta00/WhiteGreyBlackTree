package com.wgbtree.tree.wgb.operations.rotate.acc;
...
public final class GreyRotatorAcc {
	...
	Grey<K, T> tryRotateLeft(Grey<K, T> grey, TreeConfig config) {
		...
		var entry = GreyRemoverAcc.removeMinFromRight(grey);
		var leakedEntry = GreyInserterAcc.insertHere(grey, entry);
		...
		return GreyInserterAcc.insertLeft(grey, leakedKey, leakedValue, leakedKeyHash, new AtomicReference<>(), config);
	}
	...
	Grey<K, T> tryRotateRight(Grey<K, T> grey, TreeConfig config) {
		...
		var entry = GreyRemoverAcc.removeMaxFromLeft(grey);
		var leakedEntry = GreyInserterAcc.insertHere(grey, entry);
		...
		return GreyInserterAcc.insertRight(grey, leakedKey, leakedValue, leakedKeyHash,new AtomicReference<>(), config);
	}
}
