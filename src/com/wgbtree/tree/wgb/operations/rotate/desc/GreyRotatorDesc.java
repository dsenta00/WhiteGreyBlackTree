package com.wgbtree.tree.wgb.operations.rotate.desc;
...
public final class GreyRotatorDesc {
	...
	Grey<K, T> tryRotateLeft(Grey<K, T> grey, TreeConfig config) {
		...
		var entry = GreyRemoverDesc.removeMinFromRight(grey);
		var leakedEntry = GreyInserterDesc.insertHere(grey, entry);
		...
		return GreyInserterDesc.insertLeft(grey, leakedKey, leakedValue, leakedKeyHash, new AtomicReference<>(), config);
	}
	...
	Grey<K, T> tryRotateRight(Grey<K, T> grey, TreeConfig config) {
		...
		var entry = GreyRemoverDesc.removeMaxFromLeft(grey);
		var leakedEntry = GreyInserterDesc.insertHere(grey, entry);
		...
		return GreyInserterDesc.insertRight(grey, leakedKey, leakedValue, leakedKeyHash,new AtomicReference<>(), config);
	}
}
