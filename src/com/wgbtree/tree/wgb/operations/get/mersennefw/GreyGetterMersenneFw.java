...
import com.wgbtree.tree.wgb.operations.get.range.BlackGetterRange;
import com.wgbtree.tree.wgb.operations.get.range.WhiteGetterRange;
...
public final class GreyGetterMersenneFw {
    ...
    Entry<K, Set<T>> getMin(Grey<K, T> grey) {
        ...
        if (left != null) {
            if (grey instanceof FwGrey<K, T>) {
                return WhiteGetterMersenneFw.getMin(left);
            } else {
    ...
    Entry<K, Set<T>> getMax(Grey<K, T> grey) {
        ...
        if (right != null) {
            if (grey instanceof FwGrey<K, T>) {
                return BlackGetterMersenneFw.getMax(right);
            } else {
    ...
    List<Entry<K, Set<T>>> getAllAsc(FwGrey<K, T> grey) {
    ...
    public static <T, K extends Comparable<K>>
    void getAllAsc(List<Entry<K, Set<T>>> list, FwGrey<K, T> grey) {
        if (isNull(grey)) {
            return;
        }

        var left = (FwMersenneWhite<K, T>) grey.getLeft();
        WhiteGetterMersenneFw.getAllAsc(list, left);

        list.addAll(grey.getEntries());

        var right = (FwMersenneBlack<K, T>) grey.getRight();
        BlackGetterMersenneFw.getAllAsc(list, right);
    }

    public static <T, K extends Comparable<K>>
    List<Entry<K, Set<T>>> getAllDesc(FwGrey<K, T> grey) {
        List<Entry<K, Set<T>>> list = new LinkedList<>();
    ...
    public static <T, K extends Comparable<K>>
    void getAllDesc(List<Entry<K, Set<T>>> list, FwGrey<K, T> grey) {
        if (isNull(grey)) {
            return;
        }

        BlackGetterMersenneFw.getAllDesc(list, (FwMersenneBlack<K, T>) grey.getRight());

        for (int i = grey.getEntries().size() - 1; i >= 0; i--) {
            list.add(grey.getEntries().get(i));
        }

        WhiteGetterMersenneFw.getAllDesc(list, (FwMersenneWhite<K, T>) grey.getLeft());
    }

    public static <T, K extends Comparable<K>>
    List<Set<T>> getInAsc(FwGrey<K, T> grey, List<K> keys) {
        throw new UnsupportedOperationException("Not implemented yet");
    }

    public static <T, K extends Comparable<K>>
    List<Entry<K, Set<T>>> getBetweenAsc(FwGrey<K, T> grey, K from, K to) {
        if (isNull(grey)) {
            return List.of();
        }
    ...
    void getBetweenAsc(List<Entry<K, Set<T>>> list, FwGrey<K, T> grey, K from, K to) {
        if (isNull(grey)) {
            return;
        }
        ...
        if (firstKey.compareTo(from) > 0) {
            WhiteGetterMersenneFw.getBetweenAsc(list, (FwMersenneWhite<K, T>) grey.getLeft(), from, to);
        } else {
        ...
        if (lastKey.compareTo(to) < 0) {
            BlackGetterMersenneFw.getBetweenAsc(list, (FwMersenneBlack<K, T>) grey.getRight(), from, to);
        }
    }
}
