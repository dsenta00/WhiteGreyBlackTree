package com.wgbtree.tree.wgb.operations.get.asc;
...
public final class GreyGetterAsc {
	...
	Entry<K, Set<T>> getMin(Grey<K, T> grey) {
		...
		if (left != null) {
			return WhiteGetterAsc.getMin(left);
		}
	...
	Entry<K, Set<T>> getMax(Grey<K, T> grey) {
		...
		if (right != null) {
			return BlackGetterAsc.getMax(right);
		}
	...
	public static <T, K extends Comparable<K>> void getAllAsc(List<Set<T>> list, Grey<K, T> grey) {
		...
		WhiteGetterAsc.getAllAsc(list, (White<K, T>) grey.getLeft());
		grey.getEntries().forEach(entry -> list.add(entry.getValue()));
		BlackGetterAsc.getAllAsc(list, (Black<K, T>) grey.getRight());
	}
	...
	public static <T, K extends Comparable<K>> void getAllAsc(MinHeapTree<K, T> heapTree, Grey<K, T> grey) {
		...
		WhiteGetterAsc.getAllAsc(heapTree, (White<K, T>) grey.getLeft());
		grey.getEntries().forEach(heapTree::push);
		BlackGetterAsc.getAllAsc(heapTree, (Black<K, T>) grey.getRight());
	}
	...
	public static <T, K extends Comparable<K>> void getAllDesc(List<Set<T>> list, Grey<K, T> grey) {
		...
		BlackGetterAsc.getAllDesc(list, (Black<K, T>) grey.getRight());

		for (int i = grey.getEntries().size() - 1; i >= 0; i--) {
			list.add(grey.getEntries().get(i).getValue());
		}

		WhiteGetterAsc.getAllDesc(list, (White<K, T>) grey.getLeft());
	}
	...
	public static <K extends Comparable<K>, T> void getAllDesc(MaxHeapTree<K, T> heapTree, Grey<K, T> grey) {
		...
		BlackGetterAsc.getAllDesc(heapTree, (Black<K, T>) grey.getRight());
		grey.getEntries().forEach(heapTree::push);
		WhiteGetterAsc.getAllDesc(heapTree, (White<K, T>) grey.getLeft());
	}
