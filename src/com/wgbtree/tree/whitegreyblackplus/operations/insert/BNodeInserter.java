...
import com.wgbtree.tree.whitegreyblackplus.creator.BNodeCreator;
...
public final class BNodeInserter {

	public static <K extends Comparable<K>, T> BNode<K, T> insert(BNode<K, T> node, K key, Set<T> value, int keyHash, int order, int rank, AtomicReference<T> oldValue, boolean allowDuplicates) {
		if (isNull(node)) {
			return BNodeCreator.create(order, rank, key, value, allowDuplicates);
		}
